...
public class C2SToggleSchuelerStatus implements RequestPacket {

    private static final Logger logger = LoggerFactory.getLogger(C2SToggleSchuelerStatus.class);
    private long requestId;
    private String schuelerUsername;

    public C2SToggleSchuelerStatus() {
    }

    public C2SToggleSchuelerStatus(String schuelerUsername) {
    ...
    public void handle(NetworkContext ctx) {
        ...
        if (sc instanceof ServerClient && sc.getNutzer() instanceof at.tgm.objects.Lehrer) {
            logger.info("ToggleSchuelerStatus-Anfrage von Lehrer: {} für Schüler: {} (Request-ID: {})", 
                       username, schuelerUsername, requestId);
            try {
                ((ServerClient) sc).toggleSchuelerStatus(schuelerUsername, requestId);
            } catch (IOException e) {
                logger.error("Fehler beim Verarbeiten des ToggleSchuelerStatus für Lehrer: {}", username, e);
                throw new RuntimeException(e);
            }
        } else {
            logger.warn("ToggleSchuelerStatus-Anfrage von nicht-Lehrer oder nicht-ServerClient: {}", username);
        }
    }
}
